...
21. **The texview** (`.texview`, projects) — a small Overleaf standing in for a project
    ...
    `<sub>`, a `.bigsum` for stacked summation limits, `⌊ ⌋` floor glyphs), split by a
    hairline and stacked under 560px. A mono `recompile` button (`initTexview`) clears the
    output to a `compiling…` beat then re-reveals the formulas with a stagger; the static
    HTML ships both panes complete so no-JS sees source and finished render (goal-stepper
    contract). The LaTeX source must compile to exactly what the preview shows, and every
    formula is checked correct before shipping.
...
24. **Thue–Morse strips are the section separator** — `<div class="tm" data-n="24"></div>`
    stands between top-level sections rather than a plain whitespace gap or a lone hairline.
    Don't stack two strips adjacent, don't drop one between a section and its own tailpiece,
    and drop a card's redundant `border-top` when a strip now precedes it.
